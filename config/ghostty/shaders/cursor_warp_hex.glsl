...
const float SHAPE_FACTOR = 0.3; // 0.0 = tri
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    if (lineLength > minDist && baseProgress < DURATION - 0.001) {
        ...
        // Diagonal motion needs explicit side anchors so the trail keeps covering
        // the instantly-jumped current cursor instead of exposing front corners.
        float diagSideDuration = (1 - SHAPE_FACTOR) * DURATION_LEAD + SHAPE_FACTOR * DURATION_TRAIL;
